...
class StockOpnameService {
  ...
  Future<Map<String, dynamic>?> getList({
    ...
    try {
      ...
      final uri = Uri.parse('$baseUrl/api/approval-app/stock-opnames').replace(
      ...
      print('Error getList stock opname: $e');
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>?> getCreateData() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('$baseUrl/api/approval-app/stock-opnames/create-data'),
  ...
  /// Daftar user untuk approver (endpoint terpisah seperti outlet stock adjustment)
  Future<List<Map<String, dynamic>>> getApprovers(String search) async {
    try {
      final token = await _getToken();
      if (token == null) return [];
      final uri = Uri.parse('$baseUrl/api/approval-app/stock-opnames/approvers')
      ...
      print('Error getApprovers stock opname: $e');
    }
    return [];
  }

  Future<List<dynamic>> getInventoryItems({required int outletId, required int warehouseOutletId}) async {
    try {
      final token = await _getToken();
      if (token == null) return [];
      final uri = Uri.parse('$baseUrl/api/approval-app/stock-opnames/get-inventory-items').replace(
        queryParameters: {'outlet_id': outletId.toString(), 'warehouse_outlet_id': warehouseOutletId.toString()},
      );
      ...
      print('Error getInventoryItems: $e');
      return [];
    }
  }
  ...
  Future<Map<String, dynamic>?> getDetail(int id) async {
    try {
      ...
      if (token == null) return null;
      final response = await http.get(
        Uri.parse('$baseUrl/api/approval-app/stock-opnames/$id'),
        headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'},
      );
      if (response.statusCode == 200) {
        final d = jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> store({
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/api/approval-app/stock-opnames'),
  ...
  Future<Map<String, dynamic>> submitForApproval({required int id, required List<int> approvers}) async {
    try {
      ...
      if (token == null) return {'success': false, 'message': 'No token'};
      final response = await http.post(
        Uri.parse('$baseUrl/api/approval-app/stock-opnames/$id/submit-approval'),
        headers: {'Authorization': 'Bearer $token', 'Content-Type': 'application/json', 'Accept': 'application/json'},
  ...
  Future<Map<String, dynamic>> approve({required int id, required String action, String? comments}) async {
    try {
      ...
      if (token == null) return {'success': false, 'message': 'No token'};
      final response = await http.post(
        Uri.parse('$baseUrl/api/approval-app/stock-opnames/$id/approve'),
        headers: {'Authorization': 'Bearer $token', 'Content-Type': 'application/json', 'Accept': 'application/json'},
  ...
  Future<Map<String, dynamic>> process(int id) async {
    try {
      ...
      if (token == null) return {'success': false, 'message': 'No token'};
      final response = await http.post(
        Uri.parse('$baseUrl/api/approval-app/stock-opnames/$id/process'),
        headers: {'Authorization': 'Bearer $token', 'Accept': 'application/json'},
      );
      if (response.statusCode == 200) {
        final d = jsonDecode(response.body);
