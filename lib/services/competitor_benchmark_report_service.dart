...
class CompetitorBenchmarkReportService {
  static String get _prefix => '${AuthService.baseUrl}/api/approval-app/competitor-benchmark-report';
  ...
  Future<List<CbrUserOption>> searchPicUsers(String query) async {
    try {
      final token = await _token();
      if (token == null) return [];

      final uri = Uri.parse('$_prefix/search-pic-users').replace(queryParameters: {if (query.isNotEmpty) 'search': query});
      final res = await http.get(uri, headers: _headers(token));
  ...
  Future<Map<String, dynamic>> save({required Map<String, dynamic> payload, int? recordId}) async {
    try {
      final token = await _token();
      if (token == null) {
