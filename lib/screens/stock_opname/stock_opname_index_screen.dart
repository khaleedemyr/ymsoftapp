...
class StockOpnameIndexScreen extends StatefulWidget {
  const StockOpnameIndexScreen({super.key});

  @override
  State<StockOpnameIndexScreen> createState() => _StockOpnameIndexScreenState();
}

class _StockOpnameIndexScreenState extends State<StockOpnameIndexScreen> {
  ...
  Future<void> _loadCreateData() async {
    ...
    if (mounted) {
      if (result != null) {
        ...
        setState(() {
          _outlets = data['outlets'] != null && data['outlets'] is List
              ? List<Map<String, dynamic>>.from((data['outlets'] as List).map((e) => e is Map ? Map<String, dynamic>.from(e) : <String, dynamic>{}))
              : [];
          final raw = data['user_outlet_id'];
          _userOutletId = raw is int ? raw : int.tryParse(raw?.toString() ?? '');
        });
      }
  ...
  Future<void> _loadList({bool isRefresh = false}) async {
    ...
    try {
      final effectiveOutletId = (_userOutletId != null && _userOutletId != 1) ? _userOutletId : _outletIdFilter;
  ...
  void _navigateToDetail(int id) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => StockOpnameDetailScreen(opnameId: id)),
    );
  ...
  void _navigateToForm({int? editId}) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => StockOpnameFormScreen(editId: editId)),
    );
    if (result == true) _loadList(isRefresh: true);
  }
  ...
  Widget _buildFilterCard() {
    final outletSelectable = _userOutletId == null || _userOutletId == 1;
    return Container(
              ...
              children: [
                ...
                if (outletSelectable)
                  DropdownButtonFormField<int?>(
                    value: _outletIdFilter,
                    decoration: InputDecoration(
                      filled: true,
                      fillColor: const Color(0xFFF8FAFC),
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(10), borderSide: BorderSide.none),
                    ),
                    items: [const DropdownMenuItem<int?>(value: null, child: Text('Semua Outlet'))]
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text('Belum ada data stock opname', style: TextStyle(fontSize: 16, color: Colors.grey.shade600)),
          const SizedBox(height: 8),
  ...
  Widget _buildCard(Map<String, dynamic> item) {
    ...
    final outlet = item['outlet'] as Map<String, dynamic>?;
    final warehouse = item['warehouse_outlet'] as Map<String, dynamic>?;
    ...
    return Card(
      ...
      color: Colors.white,
      child: InkWell(
        onTap: id != null ? () => _navigateToDetail(id!) : null,
