...
import 'data_level_form_screen.dart';

class DataLevelIndexScreen extends StatefulWidget {
  const DataLevelIndexScreen({super.key});

  @override
  State<DataLevelIndexScreen> createState() => _DataLevelIndexScreenState();
}

class _DataLevelIndexScreenState extends State<DataLevelIndexScreen> {
  ...
  void _navigateToForm({Map<String, dynamic>? dataLevel}) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => DataLevelFormScreen(dataLevel: dataLevel),
      ),
    );
  ...
  Future<void> _confirmDelete(Map<String, dynamic> item) async {
    final name = item['nama_level'] ?? 'level ini';
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Hapus Data Level?'),
        content: Text('Yakin ingin menonaktifkan data level "$name"?'),
        actions: [
    ...
    try {
      ...
      if (mounted) {
        ...
        if (ok) {
          _loadList(isRefresh: true);
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Data Level berhasil dinonaktifkan'), backgroundColor: Colors.green),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    return AppScaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Tambah Data Level', style: TextStyle(color: Colors.white)),
      ),
      body: Column(
        children: [
          _buildToolbar(),
        ...
        children: [
          TextField(
            ...
            decoration: InputDecoration(
              hintText: 'Cari nama level / nilai level...',
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.layers_outlined, size: 72, color: Colors.grey.shade400),
          const SizedBox(height: 16),
          Text(
            _searchQuery.isNotEmpty ? 'Tidak ada hasil untuk "$_searchQuery"' : 'Belum ada data level',
          ...
          FilledButton.icon(
            ...
            label: const Text('Tambah Data Level'),
  ...
  Widget _buildCard(Map<String, dynamic> item) {
    final namaLevel = (item['nama_level'] ?? '-').toString();
    final nilaiLevel = (item['nilai_level'] ?? '-').toString();
    final nilaiPublicHoliday = item['nilai_public_holiday'] ?? 0;
    final nilaiBpjs = item['nilai_dasar_potongan_bpjs'] ?? 0;
    final nilaiPoint = item['nilai_point'] ?? 0;
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(
                  child: Text(namaLevel, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Color(0xFF1E293B))),
                ),
                GestureDetector(
            ...
            Text('Nilai Level: $nilaiLevel', style: const TextStyle(fontSize: 13, color: Color(0xFF64748B))),
            Text('Public Holiday: $nilaiPublicHoliday | BPJS: $nilaiBpjs | Point: $nilaiPoint', style: const TextStyle(fontSize: 12, color: Color(0xFF94A3B8))),
            const SizedBox(height: 12),
            Row(
              children: [
                TextButton.icon(
                  onPressed: () => _navigateToForm(dataLevel: item),
