...
class WfhRequestDetailScreen extends StatefulWidget {
  final int requestId;

  const WfhRequestDetailScreen({super.key, required this.requestId});

  @override
  State<WfhRequestDetailScreen> createState() => _WfhRequestDetailScreenState();
}

class _WfhRequestDetailScreenState extends State<WfhRequestDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    final res = await _service.getDetail(widget.requestId);
    if (!mounted) return;
    setState(() {
      _data = res?['request'] is Map
  ...
  Widget build(BuildContext context) {
    final tasks = List<Map<String, dynamic>>.from((_data?['tasks'] as List?) ?? []);
                    ...
                    _card(
                      child: Column(
                        ...
                        children: [
                          ...
                          _row('Nama', _data!['user']?['nama_lengkap']?.toString() ?? '-'),
                          _row(
                            'Jabatan',
                            _data!['user']?['jabatan']?['nama_jabatan']?.toString() ?? '-',
                          ),
                          _row(
                            'Divisi',
                            _data!['user']?['divisi']?['nama_divisi']?.toString() ?? '-',
                          ),
                      ...
                      child: Column(
                        children: flows.map((flow) {
                          ...
                          if (flowStatus == 'APPROVED') {
                            when = _formatDateTime(flow['approved_at']);
                          }
                          if (flowStatus == 'REJECTED') {
                            when = _formatDateTime(flow['rejected_at']);
                          }
                          return ListTile(
                            contentPadding: EdgeInsets.zero,
                            leading: CircleAvatar(
