...
class _UnitIndexScreenState extends State<UnitIndexScreen> {
  ...
  void _navigateToForm({Map<String, dynamic>? unit}) async {
  ...
  Future<void> _toggleStatus(Map<String, dynamic> unit) async {
    final id = unit['id'] is int ? unit['id'] as int : int.tryParse(unit['id'].toString());
    if (id == null) return;
    final current = unit['status'] ?? 'active';
  ...
  Future<void> _confirmDelete(Map<String, dynamic> unit) async {
    final name = unit['name'] ?? 'unit ini';
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Nonaktifkan Unit'),
        content: Text('Nonaktifkan unit "$name"? Data tidak dihapus permanen.'),
        actions: [
    ...
    try {
      ...
      if (mounted) {
        ...
        if (ok) {
          _loadList(isRefresh: true);
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Unit dinonaktifkan'), backgroundColor: Colors.green),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.straighten_outlined, size: 72, color: Colors.grey.shade400),
          const SizedBox(height: 16),
          Text(
            _searchQuery.isNotEmpty ? 'Tidak ada hasil untuk "$_searchQuery"' : 'Belum ada unit',
  ...
  Widget _buildCard(Map<String, dynamic> unit) {
    ...
    return Card(
      ...
      elevation: 2,
      child: InkWell(
        onTap: () => _navigateToForm(unit: unit),
          ...
          child: Row(
            children: [
              ...
              IconButton(
                ...
                icon: Icon(Icons.swap_horiz, color: isActive ? Colors.orange : Colors.green),
                tooltip: isActive ? 'Nonaktifkan' : 'Aktifkan',
              ),
              IconButton(
                onPressed: () => _navigateToForm(unit: unit),
