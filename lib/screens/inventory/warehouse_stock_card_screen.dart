...
import '../../models/warehouse_stock_card_models.dart';
import '../../services/warehouse_stock_card_service.dart';
import '../../widgets/app_loading_indicator.dart';
import '../../widgets/app_scaffold.dart';

class WarehouseStockCardScreen extends StatefulWidget {
  const WarehouseStockCardScreen({super.key});

  @override
  State<WarehouseStockCardScreen> createState() => _WarehouseStockCardScreenState();
}

class _WarehouseStockCardScreenState extends State<WarehouseStockCardScreen> {
  final WarehouseStockCardService _service = WarehouseStockCardService();
  final TextEditingController _searchController = TextEditingController();
  final TextEditingController _fromController = TextEditingController();
  final TextEditingController _toController = TextEditingController();

  List<WarehouseStockCardEntry> _cards = [];
  WarehouseStockCardSaldoAwal? _saldoAwal;
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (result != null && mounted) {
        ...
        setState(() {
          _cards = rawCards
              .map((e) => WarehouseStockCardEntry.fromJson(e as Map<String, dynamic>))
              .toList();
          _saldoAwal = saldoRaw != null ? WarehouseStockCardSaldoAwal.fromJson(saldoRaw) : null;
          _hasLoaded = true;
          _pageNum = 1;
          if (!success && message != null) _errorMessage = message;
        });
      } else {
        setState(() {
  ...
  List<WarehouseStockCardEntry> get _filteredCards {
  ...
  int get _startIndex => (_pageNum - 1) * _perPage;
  ...
  List<WarehouseStockCardEntry> get _paginatedCards {
    if (_filteredCards.isEmpty) return [];
    return _filteredCards.sublist(_startIndex, _endIndex);
  }

  void _prevPage() {
    if (_pageNum > 1) setState(() => _pageNum -= 1);
  }

  void _nextPage() {
    if (_pageNum < _totalPages) setState(() => _pageNum += 1);
  }
  ...
  String _formatQty(WarehouseStockCardEntry entry, String type) {
    if (type == 'in') {
  ...
  String _formatSaldo(WarehouseStockCardEntry entry) {
  ...
  String _buildReference(WarehouseStockCardEntry entry) {
  ...
  Future<void> _openItemSearch() async {
    ...
    if (selected != null && mounted) setState(() => _selectedItem = selected);
  }
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              labelText: 'Cari barang, gudang, referensi, keterangan',
            ...
            onChanged: (_) => setState(() => _pageNum = 1),
          ),
  ...
  Widget _buildContent() {
    ...
    if (_selectedItem == null) {
      return _buildInfoState(
        'Pilih gudang (opsional), barang, dan periode tanggal lalu klik Load Data.',
        Icons.info_outline,
        Colors.blueGrey,
      );
    }
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  const SizedBox(
                    width: 90,
                    child: Text('Referensi', style: TextStyle(fontSize: 12, color: Color(0xFF94A3B8))),
                  ),
                  Expanded(
                    child: Wrap(
              ...
              Row(
                ...
                children: [
                  const SizedBox(
                    width: 90,
                    child: Text('Keterangan', style: TextStyle(fontSize: 12, color: Color(0xFF94A3B8))),
                  ),
                  Expanded(
                    child: Text(
                      entry.description ?? '-',
...
class _ItemSearchModal extends StatefulWidget {
  final WarehouseStockCardService service;
  final Map<String, dynamic>? initialItem;

  const _ItemSearchModal({required this.service, this.initialItem});
