...
class RetailFoodDetailScreen extends StatefulWidget {
  final int retailFoodId;

  const RetailFoodDetailScreen({super.key, required this.retailFoodId});

  @override
  State<RetailFoodDetailScreen> createState() => _RetailFoodDetailScreenState();
}

class _RetailFoodDetailScreenState extends State<RetailFoodDetailScreen> {
  ...
  Future<void> _loadDetail() async {
    ...
    final result = await _service.getDetail(widget.retailFoodId);
    final canDelete = result?['canDelete'] == true || result?['can_delete'] == true;

    if (mounted) {
      setState(() {
        _data = result?['retail_food'] ?? result;
  ...
  Future<void> _deleteRetailFood() async {
    if (_isDeleting || !_canDelete || _data == null) return;
    final retailNumber = (_data?['retail_number'] ?? '-').toString();
    ...
    final result = await _service.deleteRetailFood(widget.retailFoodId);
    if (!mounted) return;
    ...
    return AppScaffold(
      ...
      actions: _isLoading
          ...
          : _canDelete
              ? [
                  IconButton(
                    onPressed: _isDeleting ? null : _deleteRetailFood,
                    icon: _isDeleting
                        ? const SizedBox(
                            width: 18,
                            height: 18,
              ...
              : [
                  IconButton(
                    ...
                    tooltip: 'Hanya admin yang dapat menghapus transaksi',
                  ),
                ],
      body: _isLoading
  ...
  Widget _buildContent() {
    ...
    final supplierName = t['supplier']?['name']?.toString() ?? t['supplier_name']?.toString() ?? '-';
        ...
        children: [
          ...
          _buildInfoCard(
            rows: [
              ...
              _InfoRow(label: 'Gudang', value: warehouseName.isNotEmpty ? warehouseName : '-'),
              _InfoRow(label: 'Supplier', value: supplierName.isNotEmpty ? supplierName : '-'),
              ...
              _InfoRow(label: 'Total', value: _formatMoney(totalAmount), valueStyle: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF2563EB))),
            ],
          ),
                  ...
                  child: Container(
                    ...
                    child: url.isEmpty
                        ...
                        : CachedNetworkImage(
                            imageUrl: url,
                            fit: BoxFit.cover,
                            placeholder: (_, __) => const Center(child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFF2563EB))),
  ...
  Widget _buildCreator(String name, String? avatarPath) {
    ...
    if (hasAvatar) {
      ...
      else avatarUrl = '${AuthService.storageUrl}/storage/${avatarPath.startsWith('/') ? avatarPath.substring(1) : avatarPath}';
    }
    return Row(
      children: [
        CircleAvatar(
