...
import 'retail_food_detail_screen.dart';
import 'retail_food_form_screen.dart';

class RetailFoodIndexScreen extends StatefulWidget {
  const RetailFoodIndexScreen({super.key});

  @override
  State<RetailFoodIndexScreen> createState() => _RetailFoodIndexScreenState();
}

class _RetailFoodIndexScreenState extends State<RetailFoodIndexScreen> {
  ...
  int? _outletId;
  int? _userOutletId; // id_outlet=1 bisa lihat semua, selain itu hanya outlet sendiri
  ...
  Future<void> _loadList({bool isRefresh = false}) async {
    ...
    try {
      // User id_outlet=1 bisa lihat semua; selain itu hanya data outlet user
  ...
  void _navigateToDetail(int id) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => RetailFoodDetailScreen(retailFoodId: id),
      ),
    );
  ...
  void _navigateToForm() async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => const RetailFoodFormScreen(),
      ),
    );
    if (result == true) _loadList(isRefresh: true);
  }
  ...
  Widget build(BuildContext context) {
    return AppScaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Tambah Retail Food', style: TextStyle(color: Colors.white)),
      ),
      body: Column(
        children: [
          _buildFilterCard(),
        ...
        children: [
          TextField(
            ...
            decoration: InputDecoration(
              hintText: 'Cari no. transaksi, supplier, outlet...',
          ...
          // Filter outlet hanya untuk user id_outlet=1 (bisa lihat semua); selain itu backend filter by user outlet
      ...
      child: Column(
        ...
        children: [
          ...
          TextButton.icon(
            ...
            label: const Text('Tambah Retail Food'),
          ),
        ],
      ),
    );
  ...
  Widget _buildCard(Map<String, dynamic> item) {
    ...
    final warehouseName = item['warehouse_outlet_name']?.toString() ?? item['warehouse_outlet_id']?.toString() ?? '-';
    final supplierName = item['supplier_name']?.toString() ?? item['supplier']?['name']?.toString() ?? '-';
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: paymentMethod == 'contra_bon' ? const Color(0xFFFEF3C7) : const Color(0xFFDCFCE7),
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            fontSize: 11,
                            fontWeight: FontWeight.w600,
                            color: paymentMethod == 'contra_bon' ? const Color(0xFFB45309) : const Color(0xFF166534),
                          ),
                        ),
                      ),
                      if (_canDelete)
                  ...
                  if (warehouseName != '-') ...[
                    const SizedBox(height: 4),
                    Text('Gudang: $warehouseName', style: const TextStyle(fontSize: 11, color: Color(0xFF94A3B8)), maxLines: 1, overflow: TextOverflow.ellipsis),
                  ],
